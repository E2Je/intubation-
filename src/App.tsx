...
export default function App() {
  const {
    ...
    isDark, toggleTheme, resetSession, itemStatuses,
  } = useChecklistStore();
  ...
  const sectionDone = sectionItems.filter(i => (itemStatuses[i.id] ?? 'pending') !== 'pending').length;
  ...
  return (
    <div className={isDark ? 'dark' : ''} style={{ height: '100dvh' }}>
      <div className="flex flex-col bg-white dark:bg-slate-950 overflow-hidden h-full">
        ...
        {/* ── Header ──────────────────────────────────────────── */}
        <header className="flex-shrink-0 bg-slate-50 dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800">

          {/* Top bar: רענון only, top-right */}
          <div className="flex justify-start px-3 pt-2 pb-0">
            <button
              onClick={() => setResetConfirmOpen(true)}
              className="text-xs px-2.5 py-1 rounded-lg font-medium transition-all bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 hover:bg-red-100 hover:text-red-600 dark:hover:bg-red-900/30 dark:hover:text-red-400"
            >
              🔄 רענון
            </button>
          </div>

          {/* Main row: branding + action buttons */}
          <div className="flex items-center justify-between px-3 pt-1 pb-2 gap-2">
            {/* Branding */}
        ...
        <div className="flex-shrink-0 px-4 pt-2 pb-1.5 border-b border-slate-100 dark:border-slate-800/60">
          <div className="flex items-center justify-between mb-1.5">
            <span className="text-slate-500 dark:text-slate-400 text-xs font-medium tabular-nums">
              {sectionDone} מתוך {sectionTotal}
            </span>
            <h2 className="text-slate-800 dark:text-slate-200 font-bold text-base">{section?.title}</h2>
          </div>
          <div className="h-1.5 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden">
            <div
              className={`h-full rounded-full transition-all duration-500 ${sectionPct === 100 ? 'bg-emerald-500' : 'bg-blue-500'}`}
