...
function calcDose(dosageStr: string, weight: number): string {
  // Handle ranges like "1-2" or "0.2-0.6"
    ...
    <div className="bg-slate-800/80 rounded-2xl p-4">
      <div className="flex items-center justify-between mb-1">
        ...
        <div className="text-right">
          <span className="text-emerald-400 font-bold text-lg">{calculated}</span>
          <span className="text-slate-400 text-sm ml-1">{item.unit.split('/')[0]}</span>
        </div>
      </div>
      ...
      <p className="text-rose-400 text-xs mt-2 leading-tight">{item.warning_note}</p>
    </div>
  );
}
...
export function MedicationDrawer() {
  ...
  return (
    <div className="fixed inset-0 z-40 flex flex-col justify-end" onClick={toggleMedDrawer}>
      <div
        className="bg-slate-900 border-t border-slate-700 rounded-t-3xl max-h-[80vh] flex flex-col shadow-2xl"
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 pb-3 border-b border-slate-800">
          <h3 className="text-white font-bold text-lg">מחשבון תרופות</h3>
          <div className="flex items-center gap-3">
            {/* Adult/Pediatric toggle */}
            <div className="flex bg-slate-800 rounded-xl p-0.5">
              <button
                onClick={() => setIsAdult(true)}
                className={`px-3 py-1.5 rounded-lg text-sm font-semibold transition-all ${isAdult ? 'bg-blue-600 text-white' : 'text-slate-400'}`}
              >
                מבוגר
              </button>
              <button
                onClick={() => setIsAdult(false)}
                className={`px-3 py-1.5 rounded-lg text-sm font-semibold transition-all ${!isAdult ? 'bg-purple-600 text-white' : 'text-slate-400'}`}
              >
                ילד
              </button>
            </div>
            <button onClick={toggleMedDrawer} className="text-slate-400 text-xl">✕</button>
          </div>
        </div>
        ...
        {weight ? (
          <div className="flex items-center justify-between px-5 py-2 bg-slate-800/50">
            <span className="text-slate-300 text-sm">משקל</span>
            <button onClick={openWeightModal} className="text-blue-400 font-bold text-lg hover:text-blue-300">
              {weight} ק"ג ✎
            </button>
          </div>
        ) : (
          <div className="px-5 py-2">
            <button onClick={openWeightModal} className="text-blue-400 text-sm">הכנס משקל</button>
          </div>
        )}
        ...
        <div className="overflow-y-auto flex-1 px-4 py-3 space-y-4">
          {MED_SECTIONS.map(section => (
            <div key={section.title}>
              <h4 className="text-slate-400 text-xs font-bold uppercase tracking-wider mb-2 px-1">
                {section.title}
              </h4>
